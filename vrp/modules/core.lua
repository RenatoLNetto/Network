...
AddEventHandler("Connect", function(Passport, source)
	...
	if Passport and Consult and Consult ~= "0" then
		PerformHttpRequest("https://discordlookup.mesalytic.moe/v1/user/"..Consult, function(Return, Response)
			if Return == 200 and Response then
				local Result = json.decode(Response)
				if Result and Result.avatar and Result.avatar.link then
					Avatar[Passport] = Result.avatar.link
				else
					Avatar[Passport] = "https://cdn.discordapp.com/embed/avatars/0.png" -- Avatar padrão
				end
			else
				Avatar[Passport] = "https://cdn.discordapp.com/embed/avatars/0.png" -- Avatar padrão
			end
...
function vRP.UpdatePlaying(Passport,Timer)
    Timer = Timer or 0
    vRP.Query("characters/TimePlaying", { Passport = Passport, Timer = Timer })
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- TIMEPLAYING
-----------------------------------------------------------------------------------------------------------------------------------------
function vRP.TimePlaying(Passport)
    return exports['oxmysql']:scalar_async('SELECT Playing FROM characters WHERE id = ?', { Passport }) or 0
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- THREADPLAYED
-----------------------------------------------------------------------------------------------------------------------------------------
CreateThread(function()
    ...
    while true do
        ...
        for Passport, _ in pairs(Players) do
            Playing[Passport] = (Playing[Passport] or 0) + (os.time() - Timer)
        end

        if os.time() - Timer >= 1 then
            for Passport, Seconds in pairs(Playing) do
                if Seconds > 0 then
